...
export default withRequestLogging('users-notifications-read-item', async (req: Request) => {
  if (req.method !== 'PUT') return new Response('Method Not Allowed', { status: 405 });
  const url = new URL(req.url);
  const parts = url.pathname.split('/');
  const userId = parts[3] || '';
  const notificationId = parts[5] || '';
  if (!userId || !notificationId) return new Response(JSON.stringify({ ok: false, error: 'Missing params' }), { status: 400 });
