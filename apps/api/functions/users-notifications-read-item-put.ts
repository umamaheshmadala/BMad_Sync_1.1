...
export default withRequestLogging('users-notifications-read-item', withRateLimit('users-notifications-read-item', { limit: 120, windowMs: 60_000 }, withErrorHandling(async (req: Request) => {
  if (req.method !== 'PUT') return new Response('Method Not Allowed', { status: 405 });
  const url = new URL(req.url);
  const parts = url.pathname.split('/');
  const userId = parts[3] || '';
  const notificationId = parts[5] || '';
  if (!userId || !notificationId) return json({ ok: false, error: 'Missing params' }, { status: 400 });
...
export const config = {
  path: '/api/users/:userId/notifications/:notificationId/read',
};
